...
describe("sessionFromClaims", () => {
  it("builds a Session from ID token claims and token metadata, including the refresh token", () => {
    const now = 1_700_000_000;
    const session = sessionFromClaims(
      { sub: "user-1", name: "Alice", email: "alice@example.com", exp: now + 3600, iat: now },
      { accessToken: "access-token", refreshToken: "refresh-token", accessTokenExpiresAt: now + 3600 },
      now,
    );
...
describe("displayNameFromSession", () => {
  it("uses the name claim when present", () => {
    const session = sessionFromClaims(
      ...
      { accessToken: "a", refreshToken: "r", accessTokenExpiresAt: 1 },
      0,
    );
...
describe("encodeSession / decodeSession", () => {
  it("round-trips a session (including refreshToken) through the encrypted cookie", async () => {
    const session = sessionFromClaims(
      { sub: "user-1", name: "Alice", email: "alice@example.com", exp: 1, iat: 1 },
      { accessToken: "access-token", refreshToken: "refresh-token", accessTokenExpiresAt: 1_700_003_600 },
      1_700_000_000,
    );
  ...
  it("returns null when refreshToken is missing from the payload", async () => {
    const { EncryptJWT } = await import("jose");
    const secret = new TextEncoder().encode("test-session-secret-0123456789ab".padEnd(32, "0").slice(0, 32));
    const incomplete = await new EncryptJWT({
      accessToken: "a",
      idTokenClaims: { sub: "user-1", email: "alice@example.com", exp: 1, iat: 1 },
