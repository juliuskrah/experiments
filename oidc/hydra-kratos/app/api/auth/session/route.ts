...
import { getHydraConfig, isAccessTokenExpired, refreshTokens } from "@/app/lib/oidc-client";
...
export async function GET(request: NextRequest) {
  ...
  // Access token expired — perform a standards-compliant OAuth2 refresh-token grant against
  // Hydra's public token endpoint (FR-006; research.md §3). Unlike Approach A, this is a real
  // refresh grant, not a silent re-authorization replay.
  let tokens;
  try {
    const config = await getHydraConfig();
    tokens = await refreshTokens(config, session.refreshToken);
  } catch {
  ...
  // The refresh grant succeeding does not by itself prove the underlying Kratos session is still
  // valid — Hydra's refresh-token store is independent of Kratos once the initial login/consent
  // bridge round-trip is complete (contracts/app-routes.md's note; spec.md Edge Cases, Scenario
  // 5b). Re-check it here, on the refresh path only, to avoid an extra network call on every
  // unexpired-access-token request.
